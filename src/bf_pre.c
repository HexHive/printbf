...
void setup() {
  ...
  real_syms = (void*)real_syms + 0x10000-(((long long int)real_syms)&0xFFFF);
  ...
  array_ = malloc(2<<24);
  array = (void*)array_ + 0x1000000-(((long long int)array_)&0xFFFFFF);
  output_ = malloc(2<<24);
  output = (void*)output_ + 0x1000000-(((long long int)output_)&0xFFFFFF);
  input_ = malloc(2<<24);
  input = (void*)input_ + 0x1000000-(((long long int)input_)&0xFFFFFF);

  progn = (void*)progn_ + 0x10000-(((long long int)progn_)&0xFFFF);

  for (int i = 0; i < sizeof(array_)/sizeof(*array); i++) {
    array_[i] = 0;
  }
  for (int i = 0; i < sizeof(progn_)/sizeof(*progn); i++) {
    progn_[i] = 0;
  }
  for (int i = 0; i < sizeof(input_)/sizeof(*input); i++) {
    input_[i] = -1;
  }
